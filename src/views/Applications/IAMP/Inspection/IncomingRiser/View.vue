        ...
        <div class="page-section">
            <div class="table-wrapper">
                ...
                >
                    ...
                    />
                    <DxColumn 
                        data-field="fieldData.pipeline_no" 
                        caption="Pipeline No." 
                        :min-width="100"
                        alignment="center" 
                    />
                    <DxColumn 
                        data-field="fieldData.riser_no" 
                        caption="Riser No." 
                        :min-width="100" 
                        alignment="center" 
                    />
                    <DxColumn 
                        data-field="fieldData.riser_type" 
                        caption="Riser Type" 
                        :min-width="100"
                        alignment="center" 
                    />
                    ...
                    <template #status-cell-template="{ data }">
                        <div>
                            <span v-if="data.data.fieldData.riser_type == 'Above Splash Zone'" >
                                {{data.data.fieldData["info::incoming_riser_status_asz"]}}
                            </span>
                            <span v-if="data.data.fieldData.riser_type == 'Subsea'">
                                {{data.data.fieldData["info::incoming_riser_status_visual_bsz"]}}
                            </span>
                        </div>
                    </template>
...
export default {
    ...
    created() {
        this.$store.commit("UPDATE_CURRENT_PAGENAME", {
            subpageName: "INSPECTION DUE INCOMING RISER",
        ...
        return {
            ...
            columns: [
                ...
                calculateCellValue: function(data) {
                    if (data.riser_type === 'Above Splash Zone') {
                    return data['info::incoming_riser_status_asz'];
                    } else if (data.riser_type === 'Subsea') {
                    return data['info::incoming_riser_status_visual_bsz'];
                    }
                    // Return a default value or empty string if no condition matches
                    return '';
                }
                }
            ]
    ...
    methods: {
        ...
        FETCH_FILE_MAKER_RECORD() {
            ...
            axiosFileMaker({
                method: "GET",
                url: "/Pipeline/layouts/info_incoming_riser_due_inspection/records?_limit=10000",
                headers: {
                    "Content-Type": "application/json",
        ...
        onCellPrepared(e) {
            console.log('data log ',e);
            if(e.rowType === "data" && e.column.caption === "Integrity Status") {
                console.log('test',e.data.fieldData);
                e.cellElement.style.backgroundColor = this.GET_STATUS_COLOR(e.data.fieldData["info::incoming_riser_status_asz"]);
