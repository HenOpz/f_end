            ...
            <div class="table-wrapper">
                <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 5px;">
                    <div class="chart-container pie-chart-wrapper">
                        <highcharts :options="chartShortTermOptions" />
                    </div>
                    <div class="chart-container pie-chart-wrapper">
                        <highcharts :options="chartShortTermDiscOptions" />
                    </div>
                </div>
                <DxDataGrid 
                    id="data-grid-list" 
...
export default {
    ...
    created() {
        ...
        if (this.$store.state.status.server == true) {
            ...
            this.chartShortTermOptions = {
                title: {
                    text: 'Short-Term Repair',
                    align: 'center'
                },
                chart: {
            ...
            this.chartShortTermDiscOptions = {
                title: {
                    text: 'Discipline',
                    align: 'center'
                },
    ...
    data() {
        return {
            testList: null,
            shortTermRecordList: {},
            chartShortTermOptions: {},
            chartShortTermDiscOptions: {},
            dataGridAttributes: {
    ...
    methods: {
        ...
        FETCH_SHORT_TERM_RECORD() {
            GET_DATA(this, '/RCFAActionRecord', (data) => {
                const list = [];
                data.forEach(data => {
        ...
        DELETE_RECORD(e) {
            DELETE_DATA(`/RCFAActionRecord/delete-rcfa-action-record?id=${e.key}`, () => { this.FETCH_SHORT_TERM_RECORD(); });
        },
