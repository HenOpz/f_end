...
export default {
  ...
  methods: {
    ...
    UPDATE_RECORD() {
      ...
      axios({
        method: "put",
        url: "/FailureActionRecord/" + this.longTermRecordList.id,
        headers: {
          Authorization: "Bearer " + JSON.parse(localStorage.getItem("token"))
        },
        data: this.longTermRecordList
      })
        .then(res => {
          if (res.status == 204) {
    ...
    DELETE_RECORD() {
      axios({
        method: "delete",
        url: "/FailureActionRecord/delete-failure-action-record?id=" + this.longTermRecordList.id,
        headers: {
          Authorization: "Bearer " + JSON.parse(localStorage.getItem("token"))
        }
      })
