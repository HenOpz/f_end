<template>
  ...
  export default {
    ...
    methods: {
      ...
      FETCH_LIST() {
        GET_DATA(this, '/Md/get-md-ex-insp-zone-list', 'dataList');
      },
      CREATE(e) {
        e.data.id = 0;
        POST_DATA('/Md/add-md-ex-insp-zone', e.data, () => { this.FETCH_LIST(); });
      },
      UPDATE(e) {
        PUT_DATA(`/Md/edit-md-ex-insp-zone?id=${e.data.id}`, e.data, () => { this.FETCH_LIST(); });
      },
      DELETE(e) {
        DELETE_DATA(`/Md/delete-md-ex-insp-zone?id=${e.data.id}`, () => { this.FETCH_LIST(); });
      },
    },
  };
  </script>
