...
export default {
  ...
  methods: {
    ...
    FETCH_LIST() {
      GET_DATA(this, '/Md/get-md-insp-campaign-status-list', 'dataList');
    },
    CREATE(e) {
      e.data.id = 0;
      POST_DATA('/Md/add-md-insp-campaign-status', e.data, () => { this.FETCH_LIST(); });
    },
    UPDATE(e) {
      PUT_DATA(`/Md/edit-md-insp-campaign-status?id=${e.data.id}`, e.data, () => { this.FETCH_LIST(); });
    },
    DELETE(e) {
      DELETE_DATA(`/Md/delete-md-insp-campaign-status?id=${e.data.id}`, () => { this.FETCH_LIST(); });
    },
  },
};
</script>
