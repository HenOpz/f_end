...
export default {
  ...
  methods: {
    ...
    FETCH_LIST() {
      GET_DATA(this, '/Md/get-md-rectification-status-list', 'dataList');
    },
    CREATE(e) {
      e.data.id = 0;
      POST_DATA('/Md/add-md-rectification-status', e.data, () => { this.FETCH_LIST(); });
    },
    UPDATE(e) {
      PUT_DATA(`/Md/edit-md-rectification-status?id=${e.data.id}`, e.data, () => { this.FETCH_LIST(); });
    },
    DELETE(e) {
      DELETE_DATA(`/Md/delete-md-rectification-status?id=${e.data.id}`, () => { this.FETCH_LIST(); });
    },
  },
};
</script>
