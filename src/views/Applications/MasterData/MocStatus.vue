...
export default {
  ...
  methods: {
    ...
    FETCH_LIST() {
      GET_DATA(this, '/Md/get-md-moc-status-list', 'dataList');
    },
    CREATE(e) {
      e.data.id = 0;
      POST_DATA('/Md/add-md-moc-status', e.data, () => { this.FETCH_LIST(); });
    },
    UPDATE(e) {
      PUT_DATA(`/Md/edit-md-moc-status?id=${e.data.id}`, e.data, () => { this.FETCH_LIST(); });
    },
    DELETE(e) {
      DELETE_DATA(`/Md/delete-md-moc-status?id=${e.data.id}`, () => { this.FETCH_LIST(); });
    },
  },
};
</script>
