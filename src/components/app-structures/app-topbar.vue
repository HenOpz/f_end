    ...
    <div id="app-topbar" v-if="canDisplayToolbar">
        <div class="app-topbar-wrapper">
            ...
            <div class="right-col" v-if="user">
                <v-ons-toolbar-button v-on:click="SHOW_POPOVER($event, 'down', true)" style="padding-right: 0px">
                ...
                <!-- POPUP PANEL -->
                <v-ons-popover cancelable :visible.sync="popoverVisible" :target="popoverTarget"
    ...
    methods: {
        async FETCH_USER_INFO() {
            ...
            if (id_account) {
                axios({
                    method: "get",
                    url: "/User/get-active-user-by-id?id=" + id_account,
                    headers: {
                    ...
                    .then(res => {
                        if (res.status == 200) {
                            console.log('fetch user topbar',res);
                            var user = res.data;
                            this.user = user;
                            if (user.img == null) {
                                this.user.img = null;
